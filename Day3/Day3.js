...
/*
    Here is Day 3 coded in javascript. This code is not commented due to the fact I used the Java version to solve this day originally.
    I did this in javascript after the fact.
    Run this with code runner.

    It takes approximitally 150 seconds to run.

    It takes only 58 seconds in Java to run.
*/
...
function findLeastDistance() {
    return finalListOfPoints.sort((a, b) => (Math.abs(a.x) + Math.abs(a.y)) - (Math.abs(b.x) + Math.abs(b.y)));
}

function findIntersection(lineOne, lineTwo) {
    for (let d of lineOne) {
        let index = lineTwo.findIndex(elm => elm.x === d.x && elm.y === d.y);
        if (index > -1) {
